...
function solution(fees, records) {
    ...
    records.forEach(record => {
        ...
        if (!cars.hasOwnProperty(num)) {
            cars[num] = {};
            cars[num]['totalTime'] = 0;
            cars[num]['IN'] = [];
            cars[num]['OUT'] = [];
        }
        ...
        if (type === 'IN') {
            cars[num]['IN'].push(time);
        } else if (type === 'OUT') {
            cars[num]['OUT'].push(time);
        }
    });

    for (let key in cars) {
        const car = cars[key];
        if (car['IN'].length !== car['OUT'].length) {
            car['OUT'].push('23:59');
        }
        ...
        car['IN'].forEach((t1, i) => {
            ...
            car['totalTime'] += (Number(h2) - Number(h1)) * 60 + Number(m2) - Number(m1);
        });

        car['fee'] = basicFee + Math.max(Math.ceil((car['totalTime'] - basicTime) / perTime), 0) * perFee;
    }

    answer = Object.keys(cars).sort((a, b) => a - b).map(key => cars[key]['fee']);
