...
function solution(id_list, report, k) {
    ...
    report.forEach(ele => {
        ...
        const from = temp[0];
        const to = temp[1];
        if (!declar.hasOwnProperty(to)) {
            declar[to] = [];
            declar[to].push(from);
        } else {
            if (!declar[to].includes(from)) {
                declar[to].push(from);
            }
        }
    });
    
    Object.keys(declar).forEach(key => {
       if (declar[key].length >= k) {
           declar[key].forEach(ele => {
               if (!id.hasOwnProperty(ele)) {
                   id[ele] = 1;
               } else {
                   id[ele]++;
               }
