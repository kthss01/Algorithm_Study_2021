...
const category = '2019 KAKAO BLIND RECRUITMENT';

const content = `
카카오톡 오픈채팅방에서는 친구가 아닌 사람들과 대화를 할 수 있는데,
본래 닉네임이 아닌 가상의 닉네임을 사용하여 채팅방에 들어갈 수 있다.

카카오톡 오픈 채팅방을 개설한 사람을 위해, 
다양한사람들이 들어오고, 나가는 것을 지켜볼 수 있는 
관리자창을 만들기로 했다.
...
ex) 채팅방에 "Muzi"와 "Prodo"라는 닉네임을 사용하는 사람이
순서대로 들어오면
"Muzi님이 들어왔습니다."
"Prodo님이 들어왔습니다."
...
Muzi가 나간 후 다시 들어올 때, Prodo라는 닉네임으로 들어올 경우
...
채팅방은 중복 닉네임을 허용하기 때문에, 현재 채팅방에는
Prodo라는 닉네임을 사용하는 사람이 두 명이 있다.
...
record가 매개변수로 주어질 때, 모든 기록이 처리된 후,
최종적으로 방을 개설한 사람이 보게 되는 메시지를 
문자열 배열 형태로 return하는 함수 구현하기
`;

const limit = `
record는 다음과 같이 문자열이 담긴 배열이며, 길이는 1 이상 100,000 이하
다음 record에 담긴 문자열에 대한 설명이다.
    모든 유저는 [유저 아이디]로 구분한다.
    [유저 아이디] 사용자가 [닉네임]으로 채팅방에 입장 
        "Enter [유저아이디] [닉네임]" ex) "Enter uid1234 Muzi"
    [유저 아이디] 사용자가 채팅방에서 퇴장
        "Leave [유저아이디]" ex) "Leave id1234"
    [유저 아이디] 사용자가 닉네임을 [닉네임]으로 변경
        "Change [유저아이디] [닉네임]" ex) "Change uid1234 Muzi"
...
const link = 'https://programmers.co.kr/learn/courses/30/lessons/42888';
...
function solution(record) {
    ...
    //     if (!user.hasOwnProperty(temp[1]))
    //         user[temp[1]] = temp[2];

    //     if (temp[0] !== 'Change') {
    //         chat.push([temp[0], temp[1]]);
    //         if (temp[0] === 'Enter') {
    //             user[temp[1]] = temp[2];
    //         }
    //     } else {
    ...
    // answer = chat.map((ele) => {
    //     let msg = `${user[ele[1]]}님이 `;
    //     msg += ele[0] === 'Enter' ? '들어왔습니다.' : '나갔습니다.';
    //     return msg;
    // });
