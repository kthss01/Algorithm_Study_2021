...
PLACE 테이블 
    공간 임대 서비스에 등록된 공간의 정보를 담는 테이블

PLACE_REVIEWS 테이블
    공간을 빌린 사람이 남긴 후기를 담는 테이블

문제
2018년 01월 01일부터 2018년 12월 31일까지 월별로 
후기가 가장 많은 공간의 이름과, 그 후기 개수를 나타내는 SQL 문을 작성해주세요. 
이때 결과는 월별로 오름차순 정렬해주세요.
단, 같은 월에 후기가 가장 많이 달린 공간이 여러 개일 경우 
공간의 이름을 기준으로 사전 순 정렬해 모두 나타내주세요.


SELECT * FROM PLACE_REVIEWS WHERE CREATED_AT BETWEEN TO_DATE('2018-01-01', 'YYYY-MM-DD') AND TO_DATE('2018-12-31', 'YYYY-MM-DD');

SELECT *
FROM PLACE_REVIEWS R RIGHT JOIN PLACES P ON R.PLACE_ID = P.ID
WHERE CREATED_AT BETWEEN TO_DATE('2018-01-01', 'YYYY-MM-DD') AND TO_DATE('2018-12-31', 'YYYY-MM-DD');

SELECT EXTRACT(MONTH FROM R.CREATED_AT) "MONTH", P.NAME, COUNT(*)
FROM PLACE_REVIEWS R RIGHT JOIN PLACES P ON R.PLACE_ID = P.ID
WHERE CREATED_AT BETWEEN TO_DATE('2018-01-01', 'YYYY-MM-DD') AND TO_DATE('2018-12-31', 'YYYY-MM-DD')
GROUP BY (EXTRACT(MONTH FROM R.CREATED_AT), P.NAME);
