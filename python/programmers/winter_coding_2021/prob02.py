...
def solution(time, gold, upgrade):
    ...
    for level in range(len(upgrade)):
        req_gold, mine_time = upgrade[level]
        
        if level != 0:
            before_req_gold, before_mine_time = upgrade[level - 1]
            if left_gold >= req_gold:
                left_gold -= req_gold
            else:
                before_time += math.ceil((req_gold - left_gold)/ gold) * before_mine_time
                if before_time > time:
                    break
                left_gold += math.ceil((req_gold - left_gold) / gold) * gold - req_gold
        
        answer = max(answer, left_gold + (time - before_time) // mine_time * gold)
